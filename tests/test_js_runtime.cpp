...
void test_error_library()
{
    ...
    // in and where it stood there — the call it was waiting in, or the
    // statement running; the script's own statements last.
    CHECK_JS_TRUE(in, "function outer() { return inner(); } function inner() { return new Error('deep').stack; }"
                      " /^Error: deep\\n    at inner \\(<test>:1:\\d+\\)\\n    at outer \\(<test>:1:27\\)\\n    at <test>:1:\\d+$/.test(outer())");
    CHECK_JS_TRUE(in, "var anonymous = function () { return (() => new TypeError('t').stack)(); };"
                      " /^TypeError: t\\n    at <test>:1:\\d+\\n    at (anonymous \\()?<test>:1:\\d+\\)?\\n    at <test>:1:\\d+$/.test(anonymous())");
    // One the engine throws itself carries the frames too, at the statement
    // that raised it rather than where its function begins.
    CHECK_JS_TRUE(in, "function reads() { try { null.x; } catch (e) { return e.stack; } } /^TypeError: .*\\n    at reads \\(<test>:1:26\\)\\n    at <test>:1:\\d+$/.test(reads())");
    CHECK_JS_TRUE(in, "function lines() {\n  var a = 1;\n  undefined.b;\n}\n"
                      "(function () { try { lines(); } catch (e) { return e.stack; } })().split('\\n')[1] === '    at lines (<test>:3:3)'");
